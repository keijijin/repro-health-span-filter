...
- `Tracer` / `ContextPropagators` は optional として受け取る
- `application.yml` から読んだ `exclude-patterns` と `propagate-context` を実際の tracer に適用
- `CamelContextConfiguration` を使い、`CamelContext` の起動直前に `openTelemetryTracer.init(camelContext)` を行う

これにより、設定値が確実に Camel 側へ流れるようにしています。

この初期化順序にした理由は、`OpenTelemetryTracer` bean 作成時に `CamelContext` を直接要求すると、
`camelContext` 生成と循環参照になりやすいためです。

また、`Tracer` bean が存在しない Java Agent 利用環境でも起動できるように、依存は optional としています。
...
- `Tracer` が存在する場合のみ `setTracer(tracer)` を設定
- `ContextPropagators` が存在する場合のみ `setContextPropagators(contextPropagators)` を設定
...
加えて、Spring Boot / Camel の起動順によっては `camelContext` と `openTelemetryTracer` の循環参照や、
`Tracer` bean 不足により Pod 起動失敗となる場合があったため、
最新実装では optional dependency と `CamelContextConfiguration` による初期化順序制御を追加しています。
...
4. `Tracer` 任意化と `CamelContextConfiguration` による初期化順序制御を追加
5. `exclude-patterns` と `propagate-context` を同時適用するサポートクラスを追加
6. テストと比較スクリプトで有効性を実証
...
- trace のつながりは維持できる
- Java Agent を使う OpenShift 環境でも起動しやすい構成にできる
