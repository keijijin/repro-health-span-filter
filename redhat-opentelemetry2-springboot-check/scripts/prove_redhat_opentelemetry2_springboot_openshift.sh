...
collect_logs() {
  local app_log="${LOG_DIR}/redhat-otel2-app.log"
  local collector_log="${LOG_DIR}/redhat-otel2-collector.log"
  oc logs deploy/redhat-otel2-app -n "${PROJECT}" > "${app_log}"
...
verify_counts() {
  ...
  if [[ "${async_count}" -gt 0 && "${process_validate_count}" -eq 0 && "${process_async_count}" -eq 0 && "${to_internal_count}" -eq 0 ]]; then
    echo "[ok] Red Hat camel-opentelemetry2 starter preserved async span propagation and suppressed processor/to output."
  else
...
echo "[start] OpenShift Red Hat opentelemetry2 starter verification"
require_cmds
setup_build
